...
{
    spinnerLoading(this, "Logging Out...", null);
    logOut();
});
...
$("#button-edit-username").click(function ()
{
    createEditModal(
        "Edit Username",
        `<input class="form-control" type="text" id="inputUsername" placeholder="Enter your new username">`,
        "username",
        "Cancel",
        "username-taken");
});

$("#button-edit-date-of-birth").click(function ()
{
    createEditModal(
        "Edit Date of Birth",
        `<input class="form-control" type="date" id="inputDateOfBirth" placeholder="Enter your new date of birth">`,
        "date-of-birth",
        "Cancel",
        "");
});

$("#button-edit-country-code").click(function ()
{
    createEditModal(
        "Edit Country Code",
        `<label for="inputCountryCode"><strong>Country Code</strong></label>
        <select id="inputCountryCode" class="form-control">
            <optgroup label="Singapore/Malaysia">
...
$("#button-edit-phone-number").click(function ()
{
    createEditModal(
        "Edit Phone Number",
...
$(document).on('click', "#button-update-username", function ()
{
    console.log("updating username...");
    spinnerLoading(this, "", null);
    validateUsername($("#inputUsername").val()).then(function (isAvailable)
    {
        if (isAvailable)
            updateField("username", $("#inputUsername").val());
        else
        {
            console.log("Not available");
            stopSpinnerLoading($(`#button-update-username`), "Update")
        }
    });
});

$(document).on('click', "#button-update-date-of-birth", function ()
{
    console.log("updating date of birth...");
    spinnerLoading(this, "", null);
    updateField("date-of-birth", $("#inputDateOfBirth").val());
});

$(document).on('click', "#button-update-country-code", function ()
{
    console.log("updating country code...");
    spinnerLoading(this, "", null);
    if (!validateCountryCode($("#inputCountryCode").val()))
    {
        stopSpinnerLoading($(`#button-update-country-code`), "Update")
        return;
    }
    else
    {
...
$(document).on('click', "#button-update-phone-number", function ()
{
    ...
    spinnerLoading(this, "", null);
    updateField("phone-number", $("#inputPhoneNumber").val());
});

function updateField(editField, updatedValue)
{
    var userInfo = firebase.database().ref('/users/' + currentUser.uid);
    ...
    switch (editField)
    {
        case "username":
            if ($("#username-profile").html())
                firebase.database().ref('/usernames/' + $("#username-profile").html())
                    .remove()
                    .then(function ()
                    {
                        var newUsername = firebase.database().ref('/usernames/' + updatedValue);
                        newUsername.set({
                            uid: currentUser.uid,
                        }).then(function ()
                        {
...
function readUserData()
{
    ...
    firebase.database().ref('/usernames/').orderByChild('uid').equalTo(currentUser.uid).on('child_added', function (snapshot)
    {
        var username;
        if (snapshot.key != "")
